...
EventLoop::EventLoop(Int_t nEvents, Float_t sqrts)
{
  ...
  macbase="/home/branden/testing/fmsu/macros/macro";
  JPsiAnalysis=false; //Only Look at e+e- from JPsi decay
  Pi0Analysis=true;
  JustTriggeredPi0=true; //Only create macros with the pi0s that pass cuts, not additional particles
  Only1Pi0=true;
//  UseMinBiasPi0=true; //Instead of only hard processes, use min bias
  UseMinBiasPi0=false; //Instead of only hard processes, use min bias
  evt_num=0;
  nev=nEvents;
  ...
  ////////////Limits
  //Generation
  etamingen=2.5; //Default 2.5
  etamaxgen=3.5;//Note these are technicaly rapdity not psudeorapdity. Negative means infinite //Default 4.4
  ptmingen=0; //pt cut for JPsi happens before initialization
  ptmaxgen=-1; //Note negative means infinite



  //Cuts
  Emincuts=3; //10; pi0, or total energy flow for min bias
  Emaxcuts=sqrts;
  ptmincuts=0;//1.0;
  ptmaxcuts=sqrts;
//  etamincuts=etamingen;
  etamincuts=2.5;
//  etamaxcuts=etamaxgen; //etamaxgen;
  etamaxcuts=3.5; //etamaxgen;
  pythia.SetMDCY(pythia.Pycomp(111),1,0); //make pi0 stable

  phimincuts=TMath::Pi()/-1.;
  phimaxcuts=TMath::Pi();
  ...
  if(Pi0Analysis)
  {
    ...
    if(!UseMinBiasPi0) pythia.SetMSEL(0); //Turn off all processes
    //Hard QCD processes
    pythia.SetMSUB(11,1);
    pythia.SetMSUB(12,1);
    pythia.SetMSUB(13,1);
...
void EventLoop::Start()
{
  ...
  for(Int_t i=0; i<nev; i++)
  {
    ...
    for(Int_t k=0; k<pythia.GetNumberOfParticles(); k++)
    {
      ...
      xHit=vz<6700 && pz>0. ? vx + px/pz *(6700. - vz) : 0;
      yHit=vz<6700. && pz>0. ? vy + py/pz *(6700. - vz) : 0;
	...
	if(etaFirst>=etamincuts && etaSecond>= etamincuts && etaFirst<=etamaxcuts && etaSecond<=etamaxcuts)
	{
	  ...
	  evt_num++;
	  outstring=Form("/home/branden/testing/fmsu/macros/macro%d.mac",evt_num);
	...
	if(KF==111 && KS<10 && pt>ptmincuts && pt<ptmaxcuts && E>Emincuts && E<Emaxcuts)
	{
      ...
      {
	//Minimum bias cuts
	////for photon, (anti)-electron, and (anti)-muon, add energy to check pt flow
	if( KS<10 && phi> phimincuts && phi<phimaxcuts &&  (KF==22 || KF==11 || KF==-11 ) )
//	if( KS<10 && xHit>=xHitmin && xHit<=xHitmax && yHit>=yHitmin && yHit<=yHitmax &&  (KF==22 || KF==11 || KF==-11) )
	{
	  ptTot+=pt;
	  energyTot+= E;
	  if(ptTot>=1.5) { keep=1; break;}
	}
      }
    }
	...
	if(eta<etamincuts || eta>etamaxcuts ) // || (KS>10 && !(KF==443 && KS==11)) || phi<phimincuts || phi>phimaxcuts || (KF!=22 && KF!=11 && KF!=-11)  )
//	if(! ( KS<10 && xHit>=xHitmin && xHit<=xHitmax && yHit>=yHitmin && yHit<=yHitmax) ) //&&  (KF==22 || KF==11 || KF==-11 ) ) )
	{
	...
	else if(!JPsiAnalysis)
	{
	  ...
	  else 
	  {
	    ...
	    else if(strcmp(PName,"pi0") == 0 || strcmp(PName,"pi-") ==0 || strcmp(PName, "pi+") == 0 || strcmp(PName, "gamma") == 0 || strcmp(PName, "e-") == 0 || strcmp(PName, "e+") == 0 || strcmp(PName, "mu-") == 0  || strcmp(PName, "mu+") == 0 ) {} // These names are already the same as geant's
	    else
	    {
	    ...
	    if(myfile.is_open())
	    {
	      ...
	      myfile<<Form("/gps/direction %f %f %F\n",px,py,pz);
	      myfile<<Form("/gps/position %f %f %f mm\n", vx, vy, vz-7000);
	      if(Pi0Analysis && JustTriggeredPi0 && Only1Pi0) break;
	    }
	  }
	}
      }
